...
class Scraper:
	...
	def __init__(self, *, retries = 3, proxies = None):
		...
		self._client = httpx.AsyncClient(http2=True, verify=True)
		self._sync_client = httpx.Client(http2=True, verify=True)
	...
	# 	print(await self._get_entity())
	# 	return await self._get_entity()

	async def entity(self):
		return await self._get_entity()

	async def _request(self, method, url, params = None, data = None, headers = None, timeout = 10, responseOkCallback = None, allowRedirects = True):
		for attempt in range(self._retries + 1):
			...
			try:
				...
				if responseOkCallback is not None:
					...
					# print(inspect.iscoroutinefunction(responseOkCallback))
					# print(responseOkCallback(r))
					# success, msg = responseOkCallback(r)
				else:
	...
	def _sync_request(self, method, url, params = None, data = None, headers = None, timeout = 10, responseOkCallback = None, allowRedirects = True):
		for attempt in range(self._retries + 1):
			# The request is newly prepared on each retry because of potential cookie updates.
			req = self._sync_client.build_request(method, url, params = params, data = data, headers = headers, timeout = timeout)
			logger.info(f'Retrieving {req.url}')
			logger.debug(f'... with headers: {headers!r}')
			if data:
				logger.debug(f'... with data: {data!r}')
	...
	def _sync_get(self, *args, **kwargs):
		return self._sync_request('GET', *args, **kwargs)

	def _sync_post(self, *args, **kwargs):
		return self._sync_request('POST', *args, **kwargs)
