...
class Tweet(snscrape.async_base.Item):
	url: str
	...
	username = snscrape.async_base._DeprecatedProperty('username', lambda self: self.user.username, 'user.username')
	outlinksss = snscrape.async_base._DeprecatedProperty('outlinksss', lambda self: ' '.join(self.outlinks) if self.outlinks else '', 'outlinks')
	tcooutlinksss = snscrape.async_base._DeprecatedProperty('tcooutlinksss', lambda self: ' '.join(self.tcooutlinks) if self.tcooutlinks else '', 'tcooutlinks')
...
@dataclasses.dataclass
class User(snscrape.async_base.Entity):
...
@dataclasses.dataclass
class Trend(snscrape.async_base.Item):
	name: str
	domainContext: str
...
class _TwitterAPIScraper(snscrape.async_base.Scraper):
	...
	# async def _ensure_guest_token(self, url = None):
	# 	if self._guestTokenManager.token is None:
	# 		_logger.info('Retrieving guest token')
	# 		r = await self._get(self._baseUrl if url is None else url, headers = {'User-Agent': self._userAgent}, responseOkCallback = self._check_guest_token_response)
	...
	# 			r = await self._post('https://api.twitter.com/1.1/guest/activate.json', data = b'', headers = self._apiHeaders, responseOkCallback = self._check_guest_token_response)
	# 			o = r.json()
	# 			if not o.get('guest_token'):
	# 				raise snscrape.async_base.ScraperError('Unable to retrieve guest token')
	...
	def _ensure_guest_token(self, url = None):
		if self._guestTokenManager.token is None:
			_logger.info('Retrieving guest token')
			r = self._sync_get(self._baseUrl if url is None else url, headers = {'User-Agent': self._userAgent}, responseOkCallback = self._check_guest_token_response)
			if (match := re.search(r'document\.cookie = decodeURIComponent\("gt=(\d+); Max-Age=10800; Domain=\.twitter\.com; Path=/; Secure"\);', r.text)):
				_logger.debug('Found guest token in HTML')
				self._guestTokenManager.token = match.group(1)
			if 'gt' in r.cookies:
			...
			if not self._guestTokenManager.token:
				_logger.debug('No guest token in response')
				_logger.info('Retrieving guest token via API')
				r = self._sync_post('https://api.twitter.com/1.1/guest/activate.json', data = b'', headers = self._apiHeaders, responseOkCallback = self._check_guest_token_response)
				o = r.json()
				if not o.get('guest_token'):
					raise snscrape.async_base.ScraperError('Unable to retrieve guest token')
				self._guestTokenManager.token = o['guest_token']
	...
	async def _check_api_response(self, r):
	...
	async def _get_api_data(self, endpoint, params):
		self._ensure_guest_token()
		r = await self._get(endpoint, params = params, headers = self._apiHeaders, responseOkCallback = self._check_api_response)
		try:
			obj = r.json()
		except json.JSONDecodeError as e:
			raise snscrape.async_base.ScraperException('Received invalid JSON from Twitter') from e
		return obj

	async def _iter_api_data(self, endpoint, params, paginationParams = None, cursor = None, direction = _ScrollDirection.BOTTOM):
		...
		while True:
			...
			obj = await self._get_api_data(endpoint, reqParams)
			yield obj
	...
	async def _instructions_to_tweets(self, obj, includeConversationThreads = False):
		# No data format test, just a hard and loud crash if anything's wrong :-)
		for instruction in obj['timeline']['instructions']:
			if 'addEntries' in instruction:
				entries = instruction['addEntries']['entries']
			...
			for entry in entries:
				if entry['entryId'].startswith('sq-I-t-') or entry['entryId'].startswith('tweet-'):
					...
					async for v in self._instruction_tweet_entry_to_tweet(entry['entryId'], entry['content'], obj):
						yield v
					...
					for item in entry['content']['timelineModule']['items']:
						if item['entryId'].startswith('tweet-'):
							async for v in self._instruction_tweet_entry_to_tweet(item['entryId'], item, obj)():
								yield v
							# yield from self._instruction_tweet_entry_to_tweet(item['entryId'], item, obj)

	async def _instruction_tweet_entry_to_tweet(self, entryId, entry, obj):
		if 'tweet' in entry['item']['content']:
			...
			raise snscrape.async_base.ScraperException(f'Unable to handle entry {entryId!r}')
...
class TwitterSearchScraper(_TwitterAPIScraper):
	...
	async def get_items(self):
		...
		async for obj in self._iter_api_data('https://api.twitter.com/2/search/adaptive.json', params, paginationParams, cursor = self._cursor):
			async for v in self._instructions_to_tweets(obj):
				yield v
		# for obj in await self._iter_api_data('https://api.twitter.com/2/search/adaptive.json', params, paginationParams, cursor = self._cursor):
		# 	async for v in self._instructions_to_tweets(obj):
		# 		yield v
	...
	def cli_setup_parser(cls, subparser):
		...
		subparser.add_argument('query', type = snscrape.async_base.nonempty_string('query'), help = 'A Twitter search string')
...
class TwitterUserScraper(TwitterSearchScraper):
	...
	async def _get_entity(self):
		...
		obj = await self._get_api_data(endpoint, params = urllib.parse.urlencode(params, quote_via=urllib.parse.quote))
		if not obj['data']:
			return None
		user = obj['data']['user']
...
class TwitterHashtagScraper(TwitterSearchScraper):
	...
	@classmethod
	def cli_setup_parser(cls, subparser):
		subparser.add_argument('hashtag', type = snscrape.async_base.nonempty_string('hashtag'), help = 'A Twitter hashtag (without #)')
...
class TwitterListPostsScraper(TwitterSearchScraper):
	...
	@classmethod
	def cli_setup_parser(cls, subparser):
		subparser.add_argument('list', type = snscrape.async_base.nonempty_string('list'), help = 'A Twitter list ID or a string of the form "username/listname" (replace spaces with dashes)')
